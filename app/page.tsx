    ...
    <div style={{ background: colors.background, minHeight: '100vh', color: colors.onSurface, fontFamily: 'Inter, sans-serif' }}>
      ...
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: isMobile ? '16px 20px' : '20px 48px', borderBottom: `1px solid ${colors.outlineVariant}`, background: colors.surfaceContainerLow }}>
        ...
        <nav style={{ display: 'flex', gap: isMobile ? 16 : 32, alignItems: 'center' }}>
          ...
          {!connected ? (<button onClick={connectWallet} style={{ background: colors.primary, color: '#fff', border: 'none', borderRadius: 8, padding: isMobile ? '6px 10px' : '10px 16px', fontWeight: 700, fontSize: isMobile ? '10px' : '12px', cursor: 'pointer' }}>Connect Wallet</button>) : (<div style={{ background: colors.surfaceContainer, border: `1px solid ${colors.outlineVariant}`, borderRadius: 8, padding: '8px 14px', fontSize: 11, fontWeight: 600, color: colors.onSurfaceVariant }}>{account?.substring(0, 6)}...{account?.substring(account ? account.length - 4 : 0)}</div>)}
        <button onClick={() => setPage('battle')} style={{ background: colors.primaryContainer, color: '#fff', border: 'none', borderRadius: 8, padding: isMobile ? '6px 12px' : '10px 24px', fontWeight: 700, fontSize: isMobile ? '11px' : '14px', cursor: 'pointer' }}>Enter Arena</button>
        </nav>
      </header>
